  ...
  return (
    <div className="w-full h-full flex flex-col gap-y-6 justify-center items-center overflow-y-auto">
      <Head>
        ...
        <meta
          name="description"
          content="Update preferences and profile settings"
        />
      </Head>
          ...
          <div className="flex flex-row items-center justify-between">
            ...
            {profile1 && (
              <div className="flex justify-between gap-8 w-2/3">
                ...
                <Button
                  variant="outline"
                  className="border-secondary1 hover:shadow-md cursor-pointer"
                  onClick={() => {
      ...
      <Card className="h-[65vh] w-7/10 bg-white dark:bg-[#18181b]">
        <CardHeader className="px-10">
          <div className="flex flex-row justify-between">
            <CardTitle className="text-4xl font-light">My Profile</CardTitle>
          </div>
          <CardDescription>
            To change your settings, press &quot;save changes&quot; after making
            edits.
          </CardDescription>
        </CardHeader>
        <CardContent className="w-full  overflow-y-scroll flex flex-col gap-y-10 px-20">
          <div className="flex flex-row items-center justify-between">
            ...
            <Input
              disabled
              type="name"
              placeholder={profile1 ? profile1.name : "name"}
              className="w-2/3 border-secondary1"
            />
          </div>
          <div className="flex flex-row items-center justify-between">
            <p>Handle:</p>
          ...
          <div className="flex flex-row items-center justify-between">
            <p>Primary Persona:</p>
            <ToggleGroup
              value={isDonator ? "donating" : "requesting"}
              type="single"
              variant="outline"
              size="sm"
              className="w-2/3"
              onValueChange={(value) => updatePersona(value)}
            >
              <ToggleGroupItem
                value="donating"
                aria-label="Toggle bold"
                className="font-bold font-hubot text-sm  p-8 py-5 shadow-sm dark:hover:text-black cursor-pointer"
              >
          ...
          <Dialog open={isOpen} onOpenChange={setIsOpen}>
            <DialogTrigger asChild>
              <Button
                disabled={!isChanged}
                variant="secondary1"
                className="hover:bg-[#cd5bdee3] hover:shadow-md"
                onClick={() => {
                  setIsOpen(true);
                }}
              >
